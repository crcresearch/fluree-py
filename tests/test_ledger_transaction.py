...
def test_ledger_transaction_single_record(request: FixtureRequest, cookbook_client: FlureeClient):
    resp = (
...
def test_ledger_transaction_multiple_records(request: FixtureRequest, cookbook_client: FlureeClient):
    resp = (
        cookbook_client.with_ledger(request.node.name)
        .transaction()
        .with_context({"ex": "http://example.org/", "schema": "http://schema.org/"})
...
# Transaction Errors
def test_transaction_on_missing_ledger(request: FixtureRequest, fluree_client: FlureeClient):
    resp = (
