...
def test_ledger_transaction(dummy_client: FlureeClient) -> None:
    request = (
...
def test_ledger_transaction_no_context(dummy_client: FlureeClient) -> None:
    request = (
        dummy_client.with_ledger("cookbook/base")
        .transaction()
        .with_where({"@id": "?s", "schema:description": "We ❤️ All Blood"})
...
def test_ledger_transaction_empty(dummy_client: FlureeClient) -> None:
    with pytest.raises(
        ValueError,
